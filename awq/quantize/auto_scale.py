...
def auto_scale_block(module, module_kwargs, w_bit, q_config, input_feat):
    ...
    def _search_module_scale(block, linears2scale: list, x, kwargs={}):
        ...
        global FC1_NUMBER, FC2_NUMBER

        if block.in_features > block.out_features:
            name = f'{FC1_NUMBER:02}_fc1'
            FC1_NUMBER += 1
        else:
            name = f'{FC2_NUMBER:02}_fc2'
            FC2_NUMBER += 1

        with open(PosixPath(OUTDIR, name + '.pkl'), mode='wb') as f:
        ...
        plt.savefig(PosixPath(OUTDIR, name + '.png'))
        plt.clf()
...
@torch.no_grad()
def auto_search_block(module, module_kwargs, w_bit, q_config, input_feat):
    from .quantizer import pseudo_quantize_tensor
    ...
    # find the best scale ratio
    def _search_module_scale(linears2scale: list, x, kwargs={}):
        # w: co, ci
        # x: n, ci

        hdn_dim = linears2scale[0].in_features
        ffn_dim = linears2scale[0].out_features

        x = x.cuda()
        with torch.no_grad():
            org_out = linears2scale[1](linears2scale[0](x), **kwargs)
            if isinstance(org_out, tuple):
                org_out = org_out[0]
        ...
        for _ in tqdm(range(1000)):
            ...
            # sample two neurons to swap
            order = list(range(ffn_dim))
            neuron_1, neuron_2 = torch.randperm(ffn_dim)[:2].tolist()
            order[neuron_1] = neuron_2
            order[neuron_2] = neuron_1

            fc1 = deepcopy(linears2scale[0])
            fc2 = deepcopy(linears2scale[1])

            fc1.weight.data = fc1.weight.data[order, :]
            fc1.bias.data = fc1.bias.data[order]
            fc2.weight.data = fc2.weight.data[:, order]

            # new_out = fc2(fc1(x, **kwargs), **kwargs)

            fc1.weight.data = w_quantize_func(fc1.weight.data)
            fc2.weight.data = w_quantize_func(fc2.weight.data)

            out = fc2(fc1(x, **kwargs), **kwargs)
            ...
            if is_best:
                best_error = loss
                # make the swap
                linears2scale[0].weight.data = linears2scale[0].weight.data[order, :]
                linears2scale[0].bias.data = linears2scale[0].bias.data[order]
                linears2scale[1].weight.data = linears2scale[1].weight.data[:, order]

                num_accepts +=1

        # print('Number of accepts', num_accepts)
