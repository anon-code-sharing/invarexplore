...
# core quantization method (simulated quantization)
def pseudo_quantize_tensor_scales_zeros(
    w, n_bit=8, zero_point=True, q_group_size=-1, inplace=False, get_scale_zp=False, scales=None, zeros=None,
):
    org_w_shape = w.shape
    if q_group_size > 0:
        assert org_w_shape[-1] % q_group_size == 0
...
@torch.no_grad()
def pseudo_quantize_model_weight_qparams(
    model,
    w_bit,
    q_config,
    qparams,
    ...
    for i in tqdm(range(len(layers)), desc="pseudo weight quantization..."):
        ...
        for n, m in named_linears.items():
            m.cuda()
            if n == 'fc2':
                scales = qparams['fc2'][i].scales
                zeros = qparams['fc2'][i].zeros
                m.weight.data = pseudo_quantize_tensor_scales_zeros(
                    m.weight.data, n_bit=w_bit,
                    scales=scales, zeros=zeros, **q_config)
            else:
                m.weight.data = pseudo_quantize_tensor(
                    m.weight.data, n_bit=w_bit, **q_config
                )
